...
// VendorSwipeModal chargé dynamiquement (ssr: false — pointer events client uniquement)
...
// ── WidgetCard — wrapper avec resize + link + snap ────────────────────────────
function WidgetCard({
  ...
  // Snap animation on resize
  useEffect(() => {
    if (size !== prevSize.current) {
      prevSize.current = size
      setSnapped(true)
      const t = setTimeout(() => setSnapped(false), 180)
  ...
  const handleResizeStart = useCallback((e: React.MouseEvent) => {
    ...
    function onMove(ev: MouseEvent) {
      ...
      const colW   = Math.max(160, cardW / startSize)
      const raw    = startSize + delta / colW
      const next   = Math.max(1, Math.min(4, Math.round(raw))) as WidgetSize
      onResize(id, next)
    }
    function onUp() {
  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        <span style={{
          fontSize: 11, opacity: hovered ? 0.45 : 0.15,
          transition: "opacity 0.15s", flexShrink: 0, color: "#45474D",
        ...
        {href ? (
          <Link
            ...
            style={{
              fontSize: 11, fontWeight: 600, color: "#121317",
              flex: 1, minWidth: 0, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis",
              textDecoration: "none", opacity: 0.9,
            }}
          >{title}</Link>
        ) : (
          <span style={{ fontSize: 11, fontWeight: 600, color: "#121317", flex: 1, minWidth: 0, opacity: 0.9 }}>
            {title}
          </span>
        )}
        {/* Arrow link + remove */}
          ...
          {removable && onRemove && (
            <button
              ...
              style={{
                ...
                background: "rgba(183,191,217,0.25)", border: "none", cursor: "pointer",
      ...
      {/* Resize handle — bottom-right, ew-resize cursor */}
      <div
...
// ── Widget picker modal ───────────────────────────────────────────────────────
function WidgetPickerModal({ active, onAdd, onClose }: {
  active: string[]; onAdd: (id: string) => void; onClose: () => void
}) {
  return (
    <div
      style={{
    ...
    >
      ...
      >
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
          <div>
            <h2 style={{ fontSize: 16, fontWeight: 800, color: "#121317", margin: "0 0 2px", letterSpacing: "-0.02em" }}>
              Ajouter un widget
            </h2>
            <p style={{ fontSize: 11, color: "#9a9aaa", margin: 0 }}>
              Personnalisez votre espace de travail
            </p>
          </div>
          <button onClick={onClose} style={{
            width: 28, height: 28, borderRadius: "50%",
            background: "rgba(183,191,217,0.12)", border: "none",
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(120px, 1fr))", gap: 10 }}>
          {WIDGET_CATALOG.map(w => {
            ...
            return (
              <button
                key={w.id}
                onClick={() => { if (!isActive) { onAdd(w.id); onClose() } }}
                disabled={isActive}
                style={{
                  padding: "14px 12px", borderRadius: 14,
                  border: isActive ? "1.5px solid rgba(34,197,94,0.3)" : "1px solid rgba(183,191,217,0.2)",
                  background: isActive ? "rgba(34,197,94,0.05)" : "rgba(183,191,217,0.04)",
              ...
              >
                ...
                <div style={{
                  ...
                  color: isActive ? "#22c55e" : "#9a9aaa",
                  fontWeight: isActive ? 600 : 400,
                }}>{isActive ? "✓ Actif" : w.category}</div>
              </button>
            )
          })}
        </div>
...
export default function CloneDashboardPage() {
  ...
  // Persist order + sizes in localStorage
  useEffect(() => {
    try {
      const savedOrder = localStorage.getItem("momento_clone_widget_order")
      const savedSizes = localStorage.getItem("momento_clone_widget_sizes")
      const savedExtra = localStorage.getItem("momento_clone_extra_widgets")
      if (savedOrder) setWidgetOrder(JSON.parse(savedOrder))
      if (savedSizes) setWidgetSizes(prev => ({ ...prev, ...JSON.parse(savedSizes) }))
      if (savedExtra) setExtraWidgets(JSON.parse(savedExtra))
    } catch {}
  }, [])

  useEffect(() => {
    try { localStorage.setItem("momento_clone_widget_order", JSON.stringify(widgetOrder)) } catch {}
  }, [widgetOrder])

  useEffect(() => {
    try { localStorage.setItem("momento_clone_widget_sizes", JSON.stringify(widgetSizes)) } catch {}
  }, [widgetSizes])

  useEffect(() => {
    try { localStorage.setItem("momento_clone_extra_widgets", JSON.stringify(extraWidgets)) } catch {}
  }, [extraWidgets])
  ...
  function addWidget(id: string) {
    setExtraWidgets(prev => [...prev, id])
    setWidgetOrder(prev => [...prev, id])
    setWidgetSizes(prev => ({ ...prev, [id]: 1 }))
  }
  function removeWidget(id: string) {
    setExtraWidgets(prev => prev.filter(w => w !== id))
    ...
    return (
      <div style={{ padding: "14px 20px 18px", height: "100%", display: "flex", flexDirection: "column", boxSizing: "border-box" }}>
            ...
            <>
              ...
              {done.map(task => (
                <div key={task.id} style={{ display: "flex", alignItems: "center", gap: 11, padding: "5px 0", borderBottom: "1px solid rgba(183,191,217,0.06)", opacity: 0.5 }}>
                  <button onClick={() => toggleTask(task.id)} className="clone-check-done" style={{
        ...
        <Link href="/clone/planner" style={{
          ...
          padding: "7px", borderRadius: 99, textDecoration: "none",
          border: "1px dashed rgba(183,191,217,0.3)", color: "#9a9aaa", fontSize: 11,
        }}>
  ...
  function renderBookings() {
    ...
    const totalAmt   = MOCK_BOOKINGS.reduce((s, b) => s + (b.amount ?? 0), 0)
    return (
      <div style={{ padding: "14px 20px 18px", height: "100%", display: "flex", flexDirection: "column", boxSizing: "border-box" }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 12 }}>
          {[
            { val: MOCK_BOOKINGS.length, label: "Prestataires", href: "/clone/explore" },
        ...
        <div style={{ flex: 1, overflowY: "auto", scrollbarWidth: "none" }}>
          {MOCK_BOOKINGS.map(b => {
            ...
            return (
              <div key={b.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 0", borderBottom: "1px solid rgba(183,191,217,0.08)" }}>
        ...
        <Link href="/clone/explore" style={{
          marginTop: 10, display: "block", padding: "7px", borderRadius: 99, textAlign: "center",
          background: G, color: "#fff", fontSize: 11, fontWeight: 600, textDecoration: "none",
        }}>+ Ajouter un prestataire</Link>
      </div>
    )
  }

  function renderMessages() {
    const unread = MOCK_MESSAGES.reduce((s, m) => s + m.unread, 0)
    return (
      <div style={{ padding: "14px 20px 18px", height: "100%", display: "flex", flexDirection: "column", boxSizing: "border-box" }}>
        {unread > 0 && (
        ...
        <div style={{ flex: 1, overflowY: "auto", scrollbarWidth: "none" }}>
          {MOCK_MESSAGES.map(msg => (
            <Link key={msg.id} href="/clone/messages" style={{
              display: "flex", alignItems: "center", gap: 11, padding: "9px 0",
              borderBottom: "1px solid rgba(183,191,217,0.08)", textDecoration: "none",
            }}>
              <div style={{
                width: 34, height: 34, borderRadius: "50%", flexShrink: 0,
                background: msg.unread > 0 ? G : "rgba(183,191,217,0.15)",
                display: "flex", alignItems: "center", justifyContent: "center",
                fontSize: 10, fontWeight: 700, color: msg.unread > 0 ? "#fff" : "#45474D",
              }}>{msg.avatar}</div>
              <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <Link href="/clone/messages" style={{
          marginTop: 10, display: "block", padding: "7px", borderRadius: 99, textAlign: "center",
          border: "1px solid rgba(183,191,217,0.25)", color: "#45474D", fontSize: 11, fontWeight: 500, textDecoration: "none",
  ...
  function renderWidgetContent(id: string) {
    switch (id as WidgetId) {
      ...
      case "budget":    return <BudgetWidget total={edata.budget} spent={edata.budgetSpent} items={BUDGET_ITEMS} />
  ...
  return (
    <div className="ant-root" style={{ display: "flex", minHeight: "100vh", background: "#f7f7fb" }}>
      ...
      {mobileOpen && (
        <div style={{ position: "fixed", inset: 0, zIndex: 50, display: "flex" }}>
          ...
          <div style={{ width: 240, height: "100%" }}>
            <DashSidebar events={EVENTS} activeEventId={activeEventId} onEventChange={id => { setActiveEventId(id); setMobileOpen(false) }} firstName={firstName} />
          </div>
        </div>
      )}

      {/* VendorSwipeModal full — avec blur */}
      {swipeOpen && (
        <div style={{ position: "fixed", inset: 0, zIndex: 70, backdropFilter: "blur(12px)", WebkitBackdropFilter: "blur(12px)", background: "rgba(18,19,23,0.7)" }}>
          <VendorSwipeModal
      ...
      {showPicker && (
        <WidgetPickerModal
          active={[...Object.keys(DEFAULT_SIZES), ...extraWidgets]}
          onAdd={addWidget}
      ...
      <main style={{ flex: 1, minWidth: 0, display: "flex", flexDirection: "column", minHeight: "100vh" }}>
        ...
        {/* Toolbar */}
        <div style={{ padding: "10px 28px 0", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <span style={{ fontSize: 10, color: "#c9cad0", display: "flex", alignItems: "center", gap: 4 }}>
            <GIcon name="drag_indicator" size={12} color="#c9cad0" />
            Glissez pour réorganiser · Poignée bas-droite pour redimensionner
          </span>
          <button
            ...
            style={{
              ...
              background: "#fff", border: "1px solid rgba(183,191,217,0.25)",
              fontSize: 11, fontWeight: 600, color: "#45474D",
              cursor: "pointer", fontFamily: "inherit",
              boxShadow: "0 1px 4px rgba(0,0,0,0.04)",
              transition: "box-shadow 0.15s",
            }}
          >
            <GIcon name="add" size={13} color="var(--g1,#E11D48)" />
            Widget
          </button>
        </div>
