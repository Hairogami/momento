...
const WIDGET_META: Record<WidgetId, { title: string; href: string; rowSpan?: number }> = {
  countdown: { title: "Compte à rebours", href: "/clone/planner"  },
  budget:    { title: "Budget",           href: "/clone/budget"   },
  swipe:     { title: "Découvrir",        href: "/clone/explore", rowSpan: 2 },
  tasks:     { title: "Tâches",           href: "/clone/planner"  },
  bookings:  { title: "Réservations",     href: "/clone/explore"  },
  messages:  { title: "Messages",         href: "/clone/messages" },
}
...
const WIDGET_CATALOG = [
  ...
  { id: "transport",   title: "Transport",              category: "Logistique"   },
...
function WidgetCard({
  ...
  return (
    ...
    >
      {/* Header — titre seul, sans icône */}
      <div style={{ display: "flex", alignItems: "center", gap: 7, padding: "13px 16px 0", userSelect: "none", flexShrink: 0 }}>
        <span style={{ fontSize: 11, opacity: hovered ? 0.4 : 0.12, transition: "opacity 0.15s", flexShrink: 0, color: "var(--dash-text-3,#9a9aaa)", fontFamily: "monospace" }}>⠿</span>
        {href ? (
          <Link href={href} onClick={e => e.stopPropagation()} style={{ fontSize: 11, fontWeight: 600, color: "var(--dash-text,#121317)", flex: 1, minWidth: 0, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", textDecoration: "none", opacity: 0.9 }}>
            {title}
          </Link>
        ) : (
          <span style={{ fontSize: 11, fontWeight: 600, color: "var(--dash-text,#121317)", flex: 1, minWidth: 0, opacity: 0.9 }}>{title}</span>
        )}
        <div style={{ display: "flex", gap: 4, flexShrink: 0, opacity: hovered ? 1 : 0, transition: "opacity 0.15s" }}>
          {href && (
            <Link href={href} onClick={e => e.stopPropagation()} style={{ textDecoration: "none", display: "flex" }}>
              <GIcon name="open_in_new" size={13} />
            </Link>
          )}
          {removable && onRemove && (
            <button
            ...
            ><GIcon name="close" size={9} /></button>
          )}
        </div>
      </div>
...
// ── Stub widget ───────────────────────────────────────────────────────────────
function StubWidget({ title }: { title: string }) {
  return (
    <div style={{ padding: "20px 24px", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", height: "100%", textAlign: "center" }}>
      <div style={{ fontSize: 12, fontWeight: 600, color: "var(--dash-text,#121317)", marginBottom: 6 }}>{title}</div>
      <div style={{ fontSize: 9, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.08em", padding: "2px 8px", borderRadius: 99, background: "rgba(245,158,11,0.1)", color: "#f59e0b" }}>Bientôt disponible</div>
    </div>
  )
}
      ...
      <div style={{ padding: "10px 16px 14px", height: "100%", display: "flex", flexDirection: "column", boxSizing: "border-box" }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 10 }}>
          {[
            ...
            { val: totalAmt > 0 ? `${(totalAmt / 1000).toFixed(0)}k MAD` : "—", label: "Engagé", href: "/clone/budget" },
          ].map(({ val, label, href }) => (
        ...
        <Link href="/clone/explore" style={{ marginTop: 8, display: "block", padding: "7px", borderRadius: 99, textAlign: "center", background: G, color: "#fff", fontSize: 11, fontWeight: 600, textDecoration: "none" }}>
  ...
  function renderMessages() {
    ...
    return (
      <div style={{ padding: "10px 16px 14px", height: "100%", display: "flex", flexDirection: "column", boxSizing: "border-box" }}>
        ...
        <div style={{ flex: 1, overflowY: "auto", scrollbarWidth: "none" }}>
          {messages.map(msg => (
            <Link key={msg.id} href="/clone/messages" style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 0", borderBottom: "1px solid var(--dash-divider)", textDecoration: "none" }}>
        ...
        <Link href="/clone/messages" style={{ marginTop: 8, display: "block", padding: "7px", borderRadius: 99, textAlign: "center", border: "1px solid var(--dash-border)", color: "var(--dash-text-2,#45474D)", fontSize: 11, fontWeight: 500, textDecoration: "none" }}>
  ...
  function renderWidgetContent(id: string) {
    switch (id as WidgetId) {
      ...
      default: {
        ...
        if (cat?.id === "notes") return <NotesWidget storageKey={`momento_notes_${activeEventId}`} />
        return cat ? <StubWidget title={cat.title} /> : null
      }
    }
  }
            ...
            {widgetOrder.map(id => {
              ...
              return (
                <WidgetCard key={id} id={id} title={meta.title} href={meta.href} size={size} rowSpan={meta.rowSpan}
                  onResize={onResize} onRemove={!isCore ? removeWidget : undefined} removable={!isCore}
