...
export default function RecommandedCarousel({ plannerId }: Props) {
  ...
  useEffect(() => {
    let cancelled = false
    setLoading(true)
    fetch(`/api/planners/${plannerId}/recommendations`)
      .then(r => r.ok ? r.json() : [])
    ...
    <section style={{ marginBottom: 28 }}>
      <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", marginBottom: 14 }}>
        <div>
          ...
          <p style={{ fontSize: 12.5, color: "var(--dash-text-3, #8888aa)", margin: "4px 0 0" }}>
            Sélection auto selon votre budget, votre ville et votre événement · 1 tap pour changer
          </p>
        </div>
        <div style={{ display: "flex", gap: 6 }}>
          <button onClick={() => scrollBy(-400)} aria-label="Précédent"
            style={navBtn}>←</button>
          <button onClick={() => scrollBy(400)} aria-label="Suivant"
            style={navBtn}>→</button>
        </div>
      </div>
      ...
      >
        ...
        {!loading && recos && recos.map(r => (
          <RecoCard key={r.category} reco={r} />
        ))}
      </div>
    </section>
  )
  ...
  return (
    <div style={cardStyle}>
      ...
      <div style={{ padding: "12px 14px 14px", flex: 1, display: "flex", flexDirection: "column" }}>
        ...
        <div style={{ marginTop: "auto", paddingTop: 10, display: "flex", gap: 5 }}>
          <Link href={`/vendor/${vendor.slug}`} style={btnView}>Voir</Link>
          <button style={btnChange}>Changer</button>
        </div>
      </div>
    </div>
  )
...
const btnView: React.CSSProperties = {
  flex: 1, padding: "7px 10px", fontSize: 11.5, color: "#fff",
  background: G, border: "none", borderRadius: 99, fontWeight: 700,
  cursor: "pointer", textAlign: "center", textDecoration: "none",
  boxShadow: "0 2px 8px rgba(225,29,72,0.25)",
}
const btnChange: React.CSSProperties = {
  padding: "7px 11px", fontSize: 11.5, color: "var(--dash-text-2, #b0b0cc)",
  background: "var(--dash-faint, rgba(255,255,255,0.04))",
  border: "1px solid var(--dash-border, rgba(255,255,255,0.07))",
  borderRadius: 99, fontWeight: 600, cursor: "pointer", fontFamily: "inherit",
}
