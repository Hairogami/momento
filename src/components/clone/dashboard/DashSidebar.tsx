...
export default function DashSidebar({ events, activeEventId, onEventChange, firstName = "Y", messageUnread = 0 }: DashSidebarProps) {
  ...
  return (
    <aside style={{
      ...
      {/* Logo */}
      <div style={{ padding: "18px 20px 16px", borderBottom: "1px solid rgba(183,191,217,0.1)" }}>
        <Link href="/clone" style={{ textDecoration: "none", display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <div>
            <div style={{ fontSize: 14, fontWeight: 800, color: "#121317", letterSpacing: "-0.03em", lineHeight: 1.1 }}>Momento</div>
            <div style={{ fontSize: 9, color: "#9a9aaa", letterSpacing: "0.06em", textTransform: "uppercase" }}>Espace client</div>
          </div>
        </Link>
      </div>

      {/* Event switcher */}
      <div style={{ padding: "10px 14px", borderBottom: "1px solid rgba(183,191,217,0.08)", position: "relative" }}>
        <button
        ...
        >
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontSize: 11, fontWeight: 600, color: "#121317", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
              {activeEvent?.name ?? "Mon événement"}
            </div>
            <div style={{ fontSize: 9, color: "#9a9aaa", marginTop: 1 }}>Événement actif</div>
          </div>
          <GIcon name="expand_more" size={14} color="#9a9aaa" />
        </button>
            ...
            {events.map(e => (
              <button
                ...
                style={{
                  ...
                  display: "flex", alignItems: "center", gap: 9,
                  background: e.id === activeEventId ? "rgba(183,191,217,0.09)" : "transparent",
                  border: "none", cursor: "pointer", fontFamily: "inherit", textAlign: "left",
                }}
              >
                <div style={{ width: 7, height: 7, borderRadius: "50%", background: e.color, flexShrink: 0 }} />
                <span style={{ fontSize: 12, fontWeight: 500, color: "#121317", flex: 1 }}>{e.name}</span>
          ...
          return (
            ...
            >
              ...
              <GIcon
                name={item.icon}
                size={18}
                color={isCurrent ? "var(--g1,#E11D48)" : "#6a6a71"}
              />
              <span style={{
                fontSize: 13, fontWeight: isCurrent ? 600 : 400,
                color: isCurrent ? "#121317" : "#45474D",
                flex: 1,
              }}>
                {item.label}
              </span>
      ...
      {/* CTA */}
      <div style={{ padding: "10px 14px", borderTop: "1px solid rgba(183,191,217,0.1)" }}>
        <Link href="/clone/explore" style={{
          display: "flex", alignItems: "center", justifyContent: "center", gap: 8,
          padding: "10px 14px", borderRadius: 11,
          background: G, color: "#fff",
          fontSize: 12, fontWeight: 600, textDecoration: "none",
          boxShadow: "0 3px 16px rgba(225,29,72,0.25)",
      ...
      <div style={{
        padding: "12px 14px",
        display: "flex", alignItems: "center", gap: 10,
        borderTop: "1px solid rgba(183,191,217,0.1)",
      }}>
        <div style={{
